"""
arb_engine.py – Arbitrage math engine with normalized Decimal odds,
three-factor priority scoring, same-match bonus detection,
and golden arb override.
"""
...
from odds_normalizer import normalize_decimal
...
class ArbEngine:
    ...
    def evaluate(
        ...
        match_name: str,
        market_key: str,
        odds_a: Tuple[str, float],
        odds_b: Tuple[str, float],
        is_live: bool,
        minutes_to_kickoff: float,
        market_display: str = "",
    ) -> Optional[ArbOpportunity]:

        plat_a, odd_a_raw = odds_a
        plat_b, odd_b_raw = odds_b
        ...
        odd_a = normalize_decimal(odd_a_raw)
        odd_b = normalize_decimal(odd_b_raw)

        # ── Implied probability ───────────────────────────────────

        imp_a = Decimal("1") / odd_a
        imp_b = Decimal("1") / odd_b
        ...
        min_margin = normalize_decimal(
            self.balancer.get_min_margin()
        )
        ...
        if is_cool:
            ...
            if not self._can_golden_override(
                margin_pct,
                self._get_speed(
                    sport,
                    market_key,
        ...
        rebalance_bonus = (
            self.balancer.get_rebalance_bonus()
        )
        ...
        stake_over, stake_under = (
            self.balancer.calculate_balanced_stakes(
                odd_a,
                odd_b,
                normalize_decimal(
